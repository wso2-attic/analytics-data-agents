...
public class DASJDriverTest {
    ...
    /**
     * Check whether the DAS Service is up and Smart home sample is running. All the test cases runs only if this is true.
     * @return
     */
    ...
    public void testSelectQuery()
    {
        try{
            ...
            //ResultSet rs = stmt.executeQuery("SELECT house_id,state,metro_area,device_id,timestamp FROM ORG_WSO2_DAS_SAMPLE_SMART_HOME_DATA where timestamp > 1455777700195;");
            //ResultSet rs = stmt.executeQuery("SELECT house_id,state,metro_area,device_id,timestamp FROM ORG_WSO2_DAS_SAMPLE_SMART_HOME_DATA where state = 'Florida';");
            //ResultSet rs = stmt.executeQuery("SELECT house_id,state,metro_area,device_id,timestamp FROM ORG_WSO2_DAS_SAMPLE_SMART_HOME_DATA where state = 'Florida';");
            //ResultSet rs = stmt.executeQuery("select tbl.HOUSE_ID,tbl.POWER_READING,tbl.DEVICE_ID,tbl.METRO_AREA,tbl.STATE,tbl.VERSION,tbl.IS_PEAK,tbl.TIMESTAMP from \"ORG_WSO2_DAS_SAMPLE_SMART_HOME_DATA\" tbl;");
            ...
            while (rs.next()){
                i++;
                System.out.println("ROW:"+i+":timestamp:"+rs.getString("timestamp")+":house_id:"+rs.getString("house_id")+"|state:"+rs.getString("state")+"|metro_area:"+rs.getString("metro_area")+"|device_id:"+rs.getInt("device_id"));
            }
            conn.close();
        }
        catch (Exception e)
    ...
    public void testSelectQueryWithAnd()
    {
        try{
            ...
            Connection conn = DriverManager.getConnection("jdbc:dasjdriver:" + url, "admin", "admin");
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("select * FROM OVERUSED_DEVICES WHERE HOUSE_ID = 15 AND METRO_AREA='San Francisco'");
            int i = 0;
            while (rs.next()){
                i++;
                System.out.println("ROW:"+i+":timestamp:"+rs.getString("timestamp")+":house_id:"+rs.getString("house_id")+"|state:"+rs.getString("state")+"|metro_area:"+rs.getString("metro_area")+"|device_id:"+rs.getInt("device_id"));
    ...
    public void testSelectQueryWithOR()
    {
        try{
            ...
            Connection conn = DriverManager.getConnection("jdbc:dasjdriver:" + url, "admin", "admin");
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("select * FROM OVERUSED_DEVICES WHERE HOUSE_ID = 15 OR METRO_AREA='San Francisco'");
            int i = 0;
            while (rs.next()){
                i++;
                System.out.println("ROW:"+i+":timestamp:"+rs.getString("timestamp")+":house_id:"+rs.getString("house_id")+"|state:"+rs.getString("state")+"|metro_area:"+rs.getString("metro_area")+"|device_id:"+rs.getInt("device_id"));
    ...
    public void testMinAggregateFunction()
    {
        try {
            ...
            ResultSet rs2 = stmt.executeQuery("select MIN(POWER_READING) FROM OVERUSED_DEVICES WHERE POWER_READING > 995");
            while (rs2.next()) {
    ...
    public void testMaxAggregateFunction()
    {
        try {
            ...
            ResultSet rs2 = stmt.executeQuery("select MAX(POWER_READING) FROM OVERUSED_DEVICES WHERE POWER_READING < 995");
            while (rs2.next()) {
