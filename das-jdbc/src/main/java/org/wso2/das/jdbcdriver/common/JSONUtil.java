...
public class JSONUtil {
    ...
    /**
     * Decode the JSON Data Array retrived from the DAS backend.
    ...
    public static DataReader parseDataArray(String sInput, HashMap<String,String> colDataTypes){
        ...
        try {
            ...
            colDataTypes.put(ServiceConstants.DAS_RESPONSE_DATA.ROW_VERSION, ServiceConstants.DATATYPES.DATATYPE_STRING); //The data type of the Column "_version" is not present in the schema query.
            ...
            for(int i =0; i<arr.size(); i++)
            {
                ...
                for (Map.Entry entry : (Set<Map.Entry>) rowValObj.entrySet()) {
                    ...
                    if(i==0) //Get the column names in the order appear in the "values" object. Extract this info only for the first row.
                    {
            ...
            dataReader.setColumnTypes(listColumnDataTypes.toArray(new String[0]));
            dataReader.setColumnNames(listColumnNames.toArray(new String[0]));
