...
public class SQLParser {
                    ...
                    if(sel.size() == 0)
                    {
                        ...
                        for(int i=0; i<sel.size(); i++) {
                            ...
                            if(aggragateName != null){
                                ...
                                Expression aggregateExpression= getAggregateFunctionExpression(aggragateName,funcExpression);
                                queryEnvironment.add(new Object[]{columnName, aggregateExpression});
                            }
                            else {
                                if (columnName.equals("*"))
                                    queryEnvironment.add(new Object[]{columnName, new AsteriskExpression(columnName)});
                                else {
                                    if (columnName.startsWith(ServiceConstants.DAS_SERVICE_QUERIES.DAS_SCHEMA_NAME + ".")) {
                                        columnName = columnName.substring(ServiceConstants.DAS_SERVICE_QUERIES.DAS_SCHEMA_NAME.length() + 1);
                                    }
                                    ...
                                    queryEnvironment.add(new Object[]{columnName, new GeneralExpression(columnName, new ColumnName(columnName))});
                                }
                            }
                        }
     ...
     */
    private Expression getWhereClauseExpression(ZExpression where){//TODO check !operator and handle operator precedence
    ...
    private RelationOpExpression getRelationOpExpression(ZExpression zExp){
        String sColName = zExp.getOperand(0).toString();//TODO: remove . in cloumn name
        ...
        return  new RelationOpExpression(zExp.getOperator(),new ColumnName(sColName),new AsteriskExpression(sColVal));//TODO:change AsteriskExpression
    }
}
